...
# introduce *coordinate reference systems*, *four packages* which allow
# tile-based map drawing, a few ways of *including user data*, and an Appendix
# on simple *coordinate conversion*. 
# The intention is to provide options to plot informative maps, which include
# the type of data collected in this unit, relatively simply. We *will not*
# cover vector-based maps, importing GIS shapefiles, interpolation of point
# data, chloropleth maps, and many other map plotting methods.

# [Find more R maps code at https://ratey-atuwa.github.io/cybloRg/mapscode.html]

#### load packages needed to make maps ####
# install them first if you don't have them on your own device
library(sf)         # state-of-the-art spatial functions
library(maptiles)   # obtain and plot open-source map tiles
library(prettymapr) # north arrow and scale bar
library(rosm)       # alternative map tile generator (not recommended)
library(ggmap)      # map tile generator in the ggplot universe
library(ggsn)       # north arrow and scale bar for ggmap
...
# library(OpenStreetMap)  # alternative map tile generator (not recommended) 
...
# We can also define our CRS using the st_crs() function from sf::
# (we need to find out the relevant EPSG codes -- see
#  https://epsg.io/ and search by country and/or projection name)

LongLat <- st_crs(4326) # uses Earth ellipsis specs from WGS84 datum
UTM50 <- st_crs(32750) # just for Zone 50, S hemisphere!
...
plot(afs19utm, add=TRUE, lwd = 2, 
                    pch = c(0,1,2,3,5,6,15,17,18,19)[afs19utm$Group], 
                    col = rainbow(10,v=0.6,end=0.75)[afs19utm$Group])
...
with(afs19utm, symbols(st_coordinates(afs19utm), add = TRUE, circles = 0.4*sqrt(Zn),
...
# define semitransparent palette based on viridis::inferno
palette(c("black", inferno(7,alpha = 0.75),"gray80","white"))
...
ggmap(afr.gg) + 
  ...
  geom_segment(aes(x = 115.9415, y = -31.9165, xend = 115.9415, yend = -31.9162),
               arrow = arrow(length = unit(0.5, "cm")), size=1.5) +
  geom_text(aes(x = 115.9415, y = -31.9166, label = "N",
                fontface="bold", family="serif"), size=5) +
  geom_sf(data = afs21ll, aes(bg=Ce, size=Ce), shape=21, inherit.aes = FALSE) +
  scale_fill_viridis_c(alpha = 0.7) + 
  scale_size_area(breaks = c(30,100,300,1000,3000,5000,10000,30000,50000),
                  max_size = 10) +
...
# ----------------------------=<>=----=<>=----=<>=---------------------------- #
#### Alternative 4 - use the OpenStreetMap package ####
# 
...
tiles_longlat <- get_tiles(longlat.temp, provider = "OpenStreetMap",
                          crop = TRUE)
plot_tiles(tiles_utm);axis(1);axis(2) # check them
plot_tiles(tiles_longlat);axis(1);axis(2)
# . . . and so on
...
afs19utm <-  st_as_sf(afs19, coords = c("Easting","Northing"), crs = UTM50)

# We then use the st_transform() function from the sf package to convert to
# long-lat, which results in another spatial data frame:
...
# Kahle D, Wickham H. ggmap: Spatial Visualization with ggplot2. The R Journal,
...
# Pedersen T, Crameri F (2023). scico: Colour Palettes Based on the Scientific
# Colour-Maps. R package version 1.4.0, https://CRAN.R-project.org/package=scico