...
#   |______|_____/_/    \_\ |_____/|______|   \/   |______|_| \_|
#     _____                                 _                    
#    / ____|                               (_)                   
#   | |     ___  _ __ ___  _ __   __ _ _ __ _ _ __   __ _        
#   | |    / _ \| '_ ` _ \| '_ \ / _` | '__| | '_ \ / _` |       
#   | |___| (_) | | | | | | |_) | (_| | |  | | | | | (_| |       
#    \_____\___/|_| |_| |_| .__/ \__,_|_|  |_|_| |_|\__, |       
#    __  __               | |                        __/ |       
#   |  \/  |              |_|                       |___/        
#   | \  / | ___  __ _ _ __  ___                                 
#   | |\/| |/ _ \/ _` | '_ \/ __|                                
...
sp(S~Na | Zone, cex=1.5, 
   ...
   legend = list(coords="topleft"), col = c(1,7,5,3,9), 
   pch = seq(14,18))
...
boxplot(log10((S*(96/32))/(Na*1.799)) ~ Zone, 
        data = afw20, horizontal = T, xaxt="n",
...
abline(v=log10(0.14), col = 12, lty = 2, lwd = 1)
text(log10(0.14) ,2, pos=4, col = 12, cex = 0.85, offset = 0.15,
     labels="Mean sea\nwater:\nSO4/Cl=0.14")
abline(v=log10(0.5), col = 2, lty = 2, lwd = 2)
text(log10(0.5) ,5, pos=4, col = 2,
  labels = "Acid\nsulfate\ntrigger!\nSO4/Cl = 0.5")
...
# Kruskal-Wallis test with Conover pairwise comps
require(PMCMRplus)
require(rcompanion)
require(multcompView)
# make new variable
afw20$SO4_Cl <- with(afw20, (S*(96/32))/(Na*1.799))
# means comparison followed by pairwise comparisons
with(afw20, kruskal.test(SO4_Cl ~ Zone))
pwSO4_Cl <- with(afw20, kwAllPairsConoverTest(SO4_Cl ~ Zone)) # from PMCMRplus
print(pwSO4_Cl)
# compact letter display again
pwSO4_Cl_pv <- fullPTable(pwSO4_Cl$p.value) # from rcompanion
multcompLetters(pwSO4_Cl_pv) # from multcompView
...
table <- data.frame(Variable = varz[10:40],
                    ...
                    Mean_lowCa = rep(NA,length(10:40)), 
                    Mean_highCa = rep(NA,length(10:40)))
