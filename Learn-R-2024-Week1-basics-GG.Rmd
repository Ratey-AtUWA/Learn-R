---
title: "R Basics with base-R and ggplot2 plotting"
output: 
...
These data are from the [Hubbard Brook Experimental Forest](https://hubbardbrook.org/experimental-watersheds-research-sites/){target="_blank"} near Woodstock in New Hampshire, USA (Figure \@ref(fig:hubbard-pix)).
...
```{r hubbard-pix, echo=FALSE, error=FALSE, fig.height=5, fig.width=13.6, fig.cap="Location of the Hubbard Brook Experimental Forest in New Hampshire, USA. The data used in this workshop were generated at this site.", warning=FALSE, results='hold'}
library(maptiles)
...
pic1 <- 
  readPNG("./images/hubbard_iconic_2.png")
plot(c(0,1),c(0,1), axes=F, type="n",xaxt="n", yaxt="n",ann=F)
...
in the `ggplot2` package. [Go to this page](Learn_R-2023-Week_1_basics.html) if
...
```{r with-plot, fig.width=3.5, fig.height=3.5, fig.cap="Plot of exchangeable Al vs. pH using with() to specify the data frame.", message=FALSE, warning=FALSE, results='hold'}
...
```{r dollar-plot, fig.width=3.5, fig.height=3.5, fig.cap="Plot of exchangeable Al vs. pH using dollar-sign syntax to specify the data frame. Notice the axis titles!", message=FALSE, warning=FALSE, results='hold'}
...
```{r plot-attach, fig.width=3.5, fig.height=3.5, fig.cap="Plot of exchangeable Al vs. pH using `attach()` to specify the data frame.", message=FALSE, warning=FALSE, echo=FALSE}
attach(hubbard)
plot(EXCH_Al ~ PH)
detach(hubbard)
```
...
```{r gg-AlEx-pH, fig.height=3.5, fig.width=3.5, message=FALSE, warning=FALSE, fig.cap="Basic plot of exchangeable Al vs. pH using the `ggplot2` R package."}
library(ggplot2)
...
Plots made using `ggplot2` such as Figure \@ref(fig:gg-AlEx-pH) set `par()`options
...
```{r plot-with-options, fig.width=4, fig.height=4, fig.cap="Plot of exchangeable Al vs. pH improved by changing graphics parameters and including custom axis titles.", message=FALSE, warning=FALSE, results='hold'}
...
see [this R colour chart](https://lms.uwa.edu.au/bbcswebdav/xid-47078256_1){target="_blank"}, or just run the R function `colors()` for a list of all 657 names!
...
```{r plot-by-groups, fig.width=4, fig.height=4, fig.cap="Plot of exchangeable Al vs. pH with custom graphics parameters and axis titles, improved by separating points by a Factor.", message=FALSE, warning=FALSE, results='hold'}
par(mar=c(4,4,1,1), mgp=c(2,0.6,0), font.lab=2)
with(hubbard,
...
```{r plot-add-legend, fig.width=4, fig.height=4, fig.cap="Plot of exchangeable Al vs. pH by sample location, with custom graphics parameters and axis titles, and a legend to explain the plot symbols.", message=FALSE, warning=FALSE, results='hold'}
par(mar=c(4,4,1,1), mgp=c(2,0.6,0), font.lab=2)
with(hubbard,
       plot(EXCH_Al ~ PH, xlab="Soil pH", 
            ylab="Exchangeable Al (centimoles/kg)",
...
```{r gg-AlEx-pH-2, fig.cap="Plot of exchangeable Al vs. pH in the Hubbard Brook soil data using the `ggplot2` R package showing points categorized by the factor `Rel.To.Brook`, and using a non-default plot theme.", fig.height=3.5, fig.width=4.8, message=FALSE, warning=FALSE}
library(ggplot2)
ggplot(data=hubbard, 
...
In the `ggplot2` example in Figure \@ref(fig:gg-AlEx-pH-2) we have used  the
`aes()` option to specify different colours and symbols depending on the value
of a factor (`Rel.To.Brook`). A legend is created automatically. We have also
used a different theme option (`theme_bw()`), which is arguably more appropriate
than the default which has a grey plot background.
...
```{r car-sp-1, fig.height=3.5, fig.width=3.5, fig.cap="Plot of exchangeable Al vs. pH made using the car::scatterplot() function.", message=FALSE, warning=FALSE, results='hold'}
...
```{r car-sp-2, fig.width=3.5, fig.height=3.5, fig.cap="Plot of exchangeable Al vs. pH with points grouped by location, made using the car::scatterplot() function.", message=FALSE, warning=FALSE, results='hold'}
...
```{r basic-hist, fig.width=3.5, fig.height=3.5, fig.cap="Histogram of percent soil moisture content at Hubbard Brook.", message=FALSE, warning=FALSE, results='hold'}
...
```{r gg-hist, fig.cap="Histogram of water content (%) in the Hubbard Brook soil data using `ggplot2`, and using a non-default ggplot theme.", fig.height=3.5, fig.width=4.8, message=FALSE, warning=FALSE}
...
```{r basic-boxplot, fig.width=5.5, fig.height=4, fig.cap="Box plot of percent soil moisture content at Hubbard Brook. Only default `boxplot()` arguments have been used (except for the annotations explaining the boxplot features).", message=FALSE, warning=FALSE, echo=FALSE, results='hold'}
par(mar=c(4,4,1,1))
...
```{r gg-boxplot, fig.cap="Boxplot of water content (%) in the Hubbard Brook soil data using `ggplot2`, and using a non-default ggplot theme.", fig.height=3.5, fig.width=4.8, message=FALSE, warning=FALSE}
...
```{r basic-mfrow, fig.width=5, fig.height=3, fig.cap="Soil organic matter content (%) at Hubbard Brook: as a one-dimensional scatterplot (`stripchart()`).", message=FALSE, warning=FALSE, results='hold'}
...
```{r gg-stripchart, fig.cap="Boxplot of water content (%) in the Hubbard Brook soil data using `ggplot2`, and using a non-default ggplot theme.", fig.height=1.5, fig.width=4.8, message=FALSE, warning=FALSE}
...
Here's an example (Table \@ref(tab:flextable-demo)) using `flextable` and the
...
flextable(data.frame(Statistic=row.names(mytable),signif(mytable,3))) |>
  ...
  set_caption(caption="Table created by the `flextable` R package. Many more table formatting, text formatting, and number formatting options are available in this package.")
```
