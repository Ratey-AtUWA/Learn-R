...
subtitle: "Multivariate methods and plots"
author: "Deidre Gleeson and Andrew Rate"
...
UTM50S <- st_crs(32750)
extent <- st_as_sf(x = data.frame(x = c(399860,400550), y = c(6467920,6468390)),
                   coords = c("x","y"), crs=UTM50S)
aftiles <- get_tiles(extent, provider="OpenStreetMap", crop=TRUE, zoom=17,
                     forceDownload = TRUE)
par(oma=c(0,0,0,0), mar=c(3,3,0.5,0.5), mgp=c(1.5,0.2,0), tcl = -0.2, 
    lend = "square", ljoin = "mitre", lheight=0.85, font.lab=2)
plot(st_coordinates(extent), asp=1, type="n", xaxs="i", yaxs="i",
     xlab="Easting (m)", ylab="Northing (m)")
plot_tiles(aftiles, mar=c(3,3,0.5,0.5), add=TRUE) ; box()
mtext("UTM Zone 50 / WGS84 (EPSG:32750)", 1, -1.2, adj=0.05, cex=0.7, col="dimgrey")
with(afr_map, polygon(wetland_E, wetland_N, col="#ADD8E6b0", lty=1, 
...
with(eDNA2023[seq(1,NROW(eDNA2023),4),], text(Easting, Northing, labels=Site, 
                  col="purple", cex = 1, font=4, offset = 0.3, 
                  pos=c(1,rep(4,16))))
```

&nbsp;

```{r species-table, message=FALSE, warning=FALSE, paged.print=FALSE, echo=1, results='hold'}
names_table <- 
  data.frame(Taxonomic = gsub("."," ",colnames(eDNA2023[,6:26]), fixed = T),
             ...
             Abbreviated=c("MosqFish", "Goby", "Mullet", "BBream", "BnjoFrog", 
                           ...
                           "MoonJely", "Dog"))
# cat('<hr style="height: 1px; background-color: #000000; width:50%; margin-left: 0; text-align:left;" />','\n')
# knitr::kable(names_table, format="html")
...
<hr style="height: 1px; background-color: black; width:50%; margin-left: 0; text-align:left;" />
...
multi-dimensional scaling, or **nmMDS**. Non-metric multidimensional scaling 
requires a complex algorithm and multiple iterations to create 2-3 new variables
(dimensions or &ldquo;axes&rdquo;) which represent the original large 
[dis]similarity matrix as well as possible. Read the page [here for a much more detailed explanation of how nmMDS works](https://archetypalecology.wordpress.com/2018/02/18/non-metric-multidimensional-scaling-nmds-what-how/){target="_blank"}! The closeness of fit of the new 2-3 dimensional 
ordination to the actual [dis]similarities is measure by the nmMDS *stress* 
parameter &ndash; see below for details.
...
analysis (Figure \@ref(fig:plot-nmds-2D)).

```{r plot-nmds-2D, fig.height=6, fig.width=6, out.width="70%", fig.align='center', fig.cap="Two-dimensional nmMDS plot, showing ordination of species and samples, for the Ashfield Flats eDNA data including DNA from all species except humans.", results='hold'}
...
The plot in Figure \@ref(fig:plot-nmds-2D) is a fairly &lsquo;typical&rsquo;
two-dimensional ordination plot. We can interpret it by assuming that samples
which are ordinated closer to one another (similar location on the plot) are
likely to be more similar than those further apart. The *species* plot at
ordination coordinates which approximately show which species contributes to the
difference of neaby samples from other samples. For example, in Figure 
\@ref(fig:plot-nmds-2D), samples at Site 9 probably differ from samples at other 
sites because they contain more DNA from Spotted Crake (and dogs!). Also, the
samples in or near the Swan River (4, 5, 6, 8) are associated with
marine/estuarine species such as mullet, goby, jellyfish, and hydroids (and,
interestingly, sheep and cattle whose DNA is probably carried downstream from
agricultural areas).
...
function (`plainPW2()` &ndash; see output in Table \@ref(tab:pairwise-Permanova) 
below).

```{r pairwise-Permanova-notrun, paged.print=FALSE, echo=2:6, eval=FALSE}
options(width = 120)
...
s0 <- data.frame(Site=1:16); flextable(cbind(s0,AF_plainPW_all))
# print(cbind(s0,AF_plainPW_all), row.names=F)
# n0 <- ceiling(sqrt(length(AF_PWpermanova_all)*2))
# AF_plainPW_all <- data.frame(Site=c(1:(n0-1)), AF_plainPW_all)
# colnames(AF_plainPW_all)[2:n0] <- c(2:n0)
# flextable(AF_plainPW_all) |> 
#   bold(j=1, bold=TRUE) |>
#   bg(j=1, bg="#c0d0ff") |> bg(j=1:17, bg="#c0d0ff", part="header") |>
#   vline(j=1, border=officer::fp_border()) |>
  # hline(j=1:17, border=officer::fp_border(), part="header") |>
  # align(align="center", part = "all") |>
  # set_caption(caption="Pairwise comparison p-values from PERMANOVA analysis of eDNA data from water samples at Ashfield Flats.", align_with_table=F, fp_p=officer::fp_par(text.align = "left", padding.bottom = 6))
```

```{r pairwise-Permanova, paged.print=FALSE, echo=FALSE}
options(width = 120)
source("https://github.com/Ratey-AtUWA/eDNA/raw/master/FUN_pairwise_adonis2.R")
source("https://raw.githubusercontent.com/Ratey-AtUWA/Learn-R/main/plain-pairwise-FUNCTION2.R")
AF_PWpermanova_all <- pairwise.adonis2(eDNA2023[,6:26] ~ Site, data = eDNA2023)
...
dissimilarities. If these are well correlated, the ordination stress will be low
and the visualisation trustworthy. Large scatter around the line suggests that
original dissimilarities are not well preserved in the reduced number of
dimensions. You would proceed to steps of plotting your NMDS (*e.g*. Figure \@ref(fig:plot-nmds-2D)\, Figure \@ref(fig:plot-nmds-2D-v2)) if you identify a
minimized stress solution &ndash; which we usually do.
