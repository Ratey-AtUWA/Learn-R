...
<li><span style="font-size: 12pt;"> Make *relevant high-quality graphics* to illustrate means comparisons\, *e.g*. box plots with means</span></li>
</ol>
...
library(car)           # utility functions like powerTransform()
library(effsize)       # effect size calculations
library(rcompanion)    # utility functions for pairwise comparisons
library(multcompView)  # utility functions for pairwise comparisons
library(PMCMRplus)     # full range of non-parametric functions incl. pairwise
```
...
```{r Kruskal-Wallis-test-wilcox-pairwise-comps, warning=FALSE, results='hold'}
with(sv2017, kwAllPairsConoverTest(Fe, Type, p.adjust.method="holm"))
```
...
pwFe_Type <- with(sv2017, kwAllPairsConoverTest(Fe, Type)) 
