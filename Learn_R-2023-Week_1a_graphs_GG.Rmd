...
<p><a href="https://lms.uwa.edu.au/bbcswebdav/pid-3577359-dt-content-rid-24052449_1/xid-24052449_1" target="_blank" rel="noreferrer noopener">![](images/html.png){alt="html file icon" style="border: 0px solid #000000;" height="32" width="32"}</a><span style="font-size: medium;"><a href="https://lms.uwa.edu.au/bbcswebdav/pid-3577359-dt-content-rid-45035919_1/xid-45035919_1" target="_blank" rel="noopener">&nbsp;<span style="background-color: #ffff40;">NEW</span>&nbsp;Annotated code for basic R and RStudio Graphics</a></span> &nbsp; &nbsp; &nbsp; <a href="https://lms.uwa.edu.au/bbcswebdav/pid-3577359-dt-content-rid-24053058_1/xid-24053058_1"><span style="font-size: medium;">![](images/rcode.png){alt="R code icon" style="border: 0px solid #000000;" height="32" width="32"}&nbsp;R code (script) file for R Graphics video 1</span></a></p>

<p><a href="https://lms.uwa.edu.au/bbcswebdav/pid-3577359-dt-content-rid-23693508_1/xid-23693508_1" target="_blank" rel="noreferrer noopener"><span style="font-size: medium;">![](images/csv.png){alt="CSV icon" style="border: 0px solid #000000;" height="32" width="32"}&nbsp;Hubbard Book soil dataset (required in code for video 1 and&nbsp; pdf)</span></a> &nbsp; &nbsp; &nbsp; <a href="https://lms.uwa.edu.au/bbcswebdav/pid-3577359-dt-content-rid-23693660_1/xid-23693660_1" target="_blank" rel="noreferrer noopener"><span style="font-size: medium;">![](images/csv.png){alt="CSV icon" style="border: 0px solid #000000;" height="32" width="32"}&nbsp;sv18 dataset (required in code for video 2)</span></a></p>

<p><span style="font-size: medium;"><a alt="" href="https://lms.uwa.edu.au/bbcswebdav/pid-3577359-dt-content-rid-45035953_1/xid-45035953_1" target="_blank" rel="noopener">![](images/csv.png){alt="CSV icon" style="border: 0px solid #000000;" height="32" width="32"}&nbsp;SL18.csv</a><br />used in Annotated code</span> &nbsp; &nbsp; &nbsp; <span style="font-size: 12pt;"><a href="https://lms.uwa.edu.au/bbcswebdav/pid-3577359-dt-content-rid-45035961_1/xid-45035961_1" target="_blank" rel="noopener">![](images/csv.png){alt="CSV icon" style="border: 0px solid #000000;" height="32" width="32"}&nbsp;sv2017_original.csv</a><br />(just in case!)</span></p>

<p><span style="font-size: medium;"><a href="https://lms.uwa.edu.au/bbcswebdav/pid-3577359-dt-content-rid-45035966_1/xid-45035966_1" target="_blank" rel="noreferrer noopener"> <span style="font-size: medium;">![](images/rcode.png){alt="R code icon" style="border: 0px solid #000000;" height="32" width="32"}&nbsp;Complete R code (script) used in video</span>&nbsp;2</a></span> &nbsp; &nbsp; &nbsp; <a href="https://lms.uwa.edu.au/bbcswebdav/pid-3577359-dt-content-rid-45035853_1/xid-45035853_1" target="_blank" style="font-family: sans-serif; text-shadow: none !important; letter-spacing: normal; margin: 0px; padding: 0px; border: 0px; outline: 0px; font-weight: 400; font-style: normal; font-size: 13px; color: #1874a4; text-decoration: underline; font-variant-ligatures: normal; font-variant-caps: normal; orphans: 2; text-align: left; text-indent: 0px; text-transform: none; white-space: normal; widows: 2; word-spacing: 0px; -webkit-text-stroke-width: 0px;" rel="noopener">![](images/rcode.png){alt="R code icon" style="border: 0px solid #000000;" height="32" width="32"}&nbsp;<span style="font-size: 12pt; font-family: sans-serif;">R code for adding mean symbols to boxplots</span></a><br /><span style="color: #ff0000;">(worth trying!)</span></p>
</div>
...
This is a supplement to the other preliminary material on **R** graphics with a
focus on using the `ggplot2` package. We have tried to avoid too much overlap
with other sessions.

For the **videos and other materials**, you can visit the 
<a href="https://lms.uwa.edu.au/bbcswebdav/pid-3355656-dt-content-rid-42740881_1/xid-42740881_1" target="_blank" rel="noopener">base-R graphics page</a>.
...
We then make an *xy*-plot (Figure \@ref(fig:ggplot-scatter-reglines)) using the 
`geom_point()` function from the `ggplot2` package, making a custom palette with 
`scale_color_viridis_d()`.

```{r ggplot-scatter-reglines, fig.height=3.5, fig.width=5, message=FALSE, warning=FALSE, fig.cap="Plot of calcium (Ca) vs. pH for different sample types from Smith's Lake and Charles Veryards Reserves in 2017 (`sv2017`), showing regression lines by Type."}
...
Adding a density line to a histogram like the one in Figure 
\@ref(fig:gg-hist-density) can help us identify *bimodal distributions*, or see 
more easily if the distribution is symmetrical or not.

```{r gg-hist-density, message=FALSE, warning=FALSE, fig.cap="Histogram of EC from the sv2017 data with density line plot added."}
...
```{r boxplot-by-groups, fig.cap="Boxplot of Cu by Type created using `ggplot2`, showing mean values as blue plus (+)  This version is drawn to resemble a `base-R` plot.", fig.height=3.5, fig.width=5, message=FALSE, warning=FALSE}
...
```{r boxplot-groups-gg2, fig.cap="Boxplot of Cu by Type created using `ggplot2`, showing mean values as blue plus (+)  This version is drawn in a style more closely resembling a `ggplot2` style.", fig.height=3.5, fig.width=5, message=FALSE, warning=FALSE}
# make an object containing a table of means
Cu_means <- data.frame(meanCu=tapply(log10(sv2017$Cu), sv2017$Type, mean, na.rm=T))
# plot the boxplot with nice axis titles etc.
...
Other important statements in the `ggplot2` code in this example (Figure 
\@ref(fig:gg-SL-water)) are:
...
```{r gg-SL-water, fig.height=5, fig.width=5, message=FALSE, warning=FALSE, fig.cap="Map-style plot of water sampling points in Smith's Lake in 2018."}
...
*e.g*. Figures \@ref(fig:ggplot-scatter-reglines), \@ref(fig:boxplot-groups-gg2), and \@ref(fig:gg-SL-water).
...
```{r Ca-pH-gg-basic, fig.height=3.5, fig.width=5, fig.cap="Default ggplot of Ca *vs*. pH.", message=FALSE, warning=FALSE}
...
```{r Ca-pH-gg-big-labels, fig.height=3.5, fig.width=5, fig.cap="Plot of Ca *vs*. pH using `ggplot2`, showing how to resize axis labels (we've also made them a different colour to be really obvious).", message=FALSE, warning=FALSE}
ggplot(data=sv18, mapping=aes(x=pH, y=Ca)) +
  geom_point() + 
  theme_bw() +
...
```{r Ca-pH-gg-resizing, fig.height=3.5, fig.width=5, fig.cap="Plot of Ca *vs*. pH using `ggplot2`, showing how to resize axis labels (and symbol sizes).", message=FALSE, warning=FALSE}
ggplot(data=sv18, mapping=aes(x=pH, y=Ca)) +
...
```{r Ca-pH-gg-logy, fig.height=3, fig.width=4, fig.cap="Plot of Ca *vs*. pH using `ggplot2`, showing how to log10-transform an axis scale.", message=FALSE, warning=FALSE}
ggplot(data=sv18, mapping=aes(x=pH, y=Ca)) +
...
factor with the property we want to change (*e.g*. `colour`, `shape`, `size`) 
in the `aes()` statement. In Figure \@ref(fig:Zn-Fe-gg-by-groups) below, we plot 
zinc (Zn) *vs*. iron (Fe), showing points in different sample types by including 
`colour=Type, shape=Type` in the `aes()` statement.
...
```{r Zn-Fe-gg-by-groups, fig.height=3, fig.width=4.5, fig.cap="Plot of Ca *vs*. pH using `ggplot2`, showing how to separate points into different groups based on factor levels.", message=FALSE, warning=FALSE}
...
The code for the plot in Figure \@ref(fig:Zn-Fe-gg-groups-recol) uses 
`scale_color_manual` to specify custom colours for the points. If we didn't 
include enough colours for the levels in the factor (*i.e*. 3 levels in `Type`), 
we would get an error message.

```{r Zn-Fe-gg-groups-recol, fig.height=3, fig.width=4.5, fig.cap="Plot of Ca *vs*. pH using `ggplot2`, showing how to separate points into different groups based on factor levels.", message=FALSE, warning=FALSE}
ggplot(data=sv18, mapping=aes(x=Fe, y=Zn, colour=Type, shape=Type)) +
  geom_point(size=2) + 
...
the same page. An example is shown in Figure \@ref(fig:ggplot-3x1).

```{r ggplot-3x1, fig.height=3, fig.width=9, fig.cap="Example of multiple `ggplot2` plots per frame using `ggarrange()` from the `ggpubr` package.", message=FALSE, warning=FALSE}
library(ggpubr)
...
```{r ecdfPlot-pH, fig.height=3, fig.width=4, fig.cap="Cumulative distribution of pH in the sv18 dataset.", message=TRUE, warning=FALSE}
...
```{r qqplot-pH, fig.cap="Normal quantile (QQ) plot of soil pH in the sv18 dataset.", fig.height=3, fig.width=4, message=FALSE, warning=FALSE}
...
```{r qq-byfactor, fig.height=3, fig.width=4.5, message=FALSE, warning=FALSE}
...
```{r hist-abline, fig.cap="Histogram of Zn in the sv18 data with a vertical line representing an environmental threshold.", fig.height=3, fig.width=4}
...
```{r gg-depth-profile, fig.height=5, fig.width=3.6, fig.cap="Plots of Fe vs. depth measurements in the sv18 data."}
...
```{r spm-ungrouped, fig.height=7, fig.width=7.5, fig.cap="Scatter plot matrix of Al, Ca, Fe, and Na concentrations in sediment + soil + street dust samples from Smith's Lake and Charles Veryard Reserves, North Perth, WA, sampled in 2018. Note the log10-transformed axes.", warning=FALSE, message=FALSE}
library(GGally)
...
looks a bit weird, unfortunately (probably because we used `scale_?_log10()` for
both *x* and *y* axes).
...
```{r spm-by-group, fig.height=6, fig.width=6, fig.cap="Scatter plot matrix of Al, Ca, Fe, and Na concentrations in samples from Smith's Lake and Charles Veryard Reserves, North Perth, WA, sampled in 2018 and grouped by sample type (sediment, soil, or street dust). Note the log10-transformed axes.", message=FALSE, warning=FALSE}
...
ggmap(slmap) + 
  ...
  annotate("text", x=c(115.85101,115.85105), y = c(-31.93215,-31.93315), 
           label = c("Inlet","Outlet"), 
           vjust = 0.5, col="steelblue4", size=3.5) + 
  scale_shape_manual(values=c(21:25,21:24)) +
...
a [separate session on maps in R
later](https://lms.uwa.edu.au/bbcswebdav/xid-45554160_1){target="_blank"} making
use of the `ggmap` **R** package, which has a lot in common with `ggplot2`
including use of `geom_sf()`.
