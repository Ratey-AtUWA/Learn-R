...
<center>
![](images/paranormal.jpg){width=300 height=300 alt="funny cartoon showing a normal distribution curve above a distribution shaped like a stereotypical ghost"}
</center>
...
(remember that you can also read the data directly from an online source)

```{r read-data-github}
hubbard <- read.csv("https://github.com/Ratey-AtUWA/Learn-R/raw/main/hubbard.csv", 
...
>
> &mdash; [Porcupine Tree](http://www.darklyrics.com/lyrics/porcupinetree/nilrecurring.html){target="_blank"}, from the song *Normal* (2007)
...
We first **load a new dataset** (we think that the variables in these data
illustrate transformations and normality testing a bit better).
...
extent <- st_as_sf(data.frame(x=c(115.846,115.8526),y=c(-31.9348,-31.9295)),
...
par(mfrow=c(1,2), oma=c(3,3,1,1), mgp=c(1.6,0.3,0), tcl=-0.25, lend="square", 
    font.lab=2)
plot_tiles(SLtiles, axes=T, cex.axis=1.4)
# axis(1);axis(2, at=seq(6466300,6466800,100), labels=seq(6466300,6466800,100))
# box()
mtext("Latitude (WGS84)", side = 1, line = 6.5, font=2, cex=1.1)
mtext("Longitude (WGS84)", side = 2, line = 5, font=2, cex=1.1)
shadowtext(c(115.8505,115.8495), c(-31.933,-31.9307),
     labels=c("Smith's\nLake\nReserve","Charles Veryard\nReserve"),
     col="darkgreen", bg="white", font=4, cex = 1.2)
mtext("(a)",3,2, adj = 0.14, cex=1.4)
pic1 <- jpeg::readJPEG("images/SmithsLakeReserve.jpg")
plot(c(0,1),c(0,1), axes=F, type="n",xaxt="n", yaxt="n",ann=F)
...
![](images/skewed.png){alt="funny cartoon of a grumpy-faced negatively skewed distribution above a happy-faced positively skewed distribution"}
