---
title: "Statistics for exploring relationships: correlation and regression"
output: 
...
## Activities for Correlation and Regression Week 1
...
<p>This gets us about half-way through correlation & regression.<br>
There’s more next week, including a short report on this topic!</p>
</td>
</tr>
<tr valign="top">
<td><span style="font-size: 12pt;"><iframe width="360" height="240" src="https://echo360.net.au/media/f444fd3e-9d67-468b-aeb2-c2b27ca26c5e/public?autoplay=false&amp;automute=false" title="E3361+Correl+Regr.mp4" allowfullscreen="allowfullscreen" webkitallowfullscreen="webkitallowfullscreen" mozallowfullscreen="mozallowfullscreen"></iframe></span></td>
<td>
<p><a href="https://github.com/Ratey-AtUWA/learningR/raw/main/sv2017_original.csv"><span style="font-size: 12pt;">💾&nbsp;The sv2017 dataset in csv (comma-separated) format</span></a></p>
...
git <- "https://github.com/Ratey-AtUWA/learningR/raw/main/"
...
Figure \@ref(fig:Cd-Zn-scatter):

```{r Cd-Zn-scatter, fig.height=6, fig.width=6, fig.cap="Scatterplot showing the relationship between Cd and Zn, with observations identified by Type but showing the regression line for all data independent of grouping.", results='hold'}
palette(c("black",viridis::cividis(8)[2:7],"white"))
...
# . . . then plot the data
plot(sv2017$Cd~sv2017$Zn, col=c(6,3,1)[sv2017$Type], log="xy", 
...
```{r correlation matrix using Hmisc, results='hold'}
...
[Note that for Pearson correlations we instead use `type="pearson"` in the 
`rcorr` function.]
...
We can do this using the `rcorr.adjust()` function in the `RcmdrMisc` package
which calculates **corrected** P-values. As above,the p-values give the
probability of the observed relationship if # the null hypothesis (*i.e*. no
relationship) is true. Corrections are to reduce the risk of **Type 1 Errors
(false positives)** which is greater when multiple comparisons are being made.

We include `use="pairwise.complete"` in the `rcorr.adjust()` function
parameters, otherwise the correlations are calculated on the number of
observations for the variable with the most missing values (try changing it
...
cormat0 <- rcorr.adjust(sv2017[c("pH","Al","Ca","Fe","Cd","Pb","Zn")], 
                        type="spearman", use="pairwise.complete")
cormat0
```

The output from `rcorr.adjust()` has a fourth sub-table:

4. the adjusted p-values for each pair of variables, which depend on the number of variables being compared
...
display the text correlation matrix after the heatmap 
(Figure \@ref(fig:correl-heat)) for reference.

```{r correl-heat, fig.height=6, fig.width=6, message=FALSE, warning=FALSE, paged.print=FALSE, fig.cap="Correlation heatmap for selected variables in the 2023 Ashfield Flats sediment data. The numeric correlation matrix is below.", results='hold'}
afs23 <- read.csv(paste0(git,"afs23.csv"), stringsAsFactors = TRUE)
library(corrplot)
cormat1 <- rcorr.adjust(afs23[,c("pH","EC","Al","Ca","Fe","Cu","Gd","Pb","Zn")], 
                        type="spearman", use="pairwise.complete")
print(cormat1$R$r, digits=2)
...
```{r scatter-plot-matrix, fig.height=10, fig.width=10, fig.cap="Scatter plot matrix for selected variables in the 2017 Smiths-Veryard sediment data, with observations and regression lines grouped by sample Type. Scatter plot matrices are a powerful exploratory data analysis tool.", results='hold'}
...
scatterplotMatrix(~pH+ log10(Al)+ log10(Ca)+ log10(Cd)+ 
                  ...
                  legend=list(coords="bottomleft"), cex=1.4, cex.axis=1.3)
```

In Figure \@ref(fig:scatter-plot-matrix), Pb and Zn are positively correlated
(r=0.77, p<0.0001) independent of which group (Sediment, Soil, or Street dust)
observations are from. Conversely, the relationship between pH and Cd is weak
(r=0.23, adjusted p=0.32), but there appear to be closer relationships between
observations from individual groups. Finally, the relationship between Al and Ca
may be influenced by a single observation with low Al and high Ca. All issues
like this should be considered when we explore our data more deeply.
...
>
> --- [Alan Alda](https://www.imdb.com/name/nm0000257/){target="_blank"}
...
<center>![](cats-lightning-correlation.jpg)</center>
...
**capacity, and the major elements** as predictors 
...
Pearson's r &ge; 0.8 or r &le; &minus;0.8, then the collinearity is too large
and one of the correlated predictors should be omitted**. In the correlation
table above this applies to the correlation between [transformed] Ca and Mg,
with **r=0.85**. In this example we will run two versions of the model, one
...
when we reduce the number of predictors we may not have the same number of
observations for all models, in which case we can't compare them.
...
investigation, while serious multi-collinearity exists
**requiring correction if VIF > 10** (Hebbali, 2018). As we probably expected 
from the correlation coefficient (above), VIFs for both Ca and Mg are >4 in this 
model, which may be OK, but we will try a model which omits Ca or Mg (we'll 
choose Mg)...
...
We don't want to have too many predictors in our model -- just the predictors
which explain significant proportions of the variance in our dependent variable.
In addition, our data may be insufficient to generate a very complex model; one
rule-of-thumb suggests 10-20 observations are needed to calculate coefficients
for each predictor. Reimann *et al*. (2008) recommend that the number of
observations should be at least 5 times the number of predictors. So, we use a
systematic stepwise procedure to test variations of the model, which omits
