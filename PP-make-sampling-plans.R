...
pp_samparea <- st_read("sampling_area.kml") |> st_transform(crs=st_crs(32750))

sa1 <- as.matrix(st_coordinates(pp_samparea)[,1:2])
sah1 <- st_read("rushes.kml") |> st_transform(crs=st_crs(32750)) |> 
  st_coordinates() |> as.matrix()
sah2 <- st_read("dense-veg.kml") |> st_transform(crs=st_crs(32750)) |> 
  st_coordinates() |> as.matrix()
sah3 <- st_read("depot.kml") |> st_transform(crs=st_crs(32750)) |> 
  st_coordinates() |> as.matrix()
samppoly <- st_polygon(list(sa1,sah1[,1:2],sah2[,1:2],sah3[,1:2])) 
samppoly <- st_sfc(samppoly, crs=st_crs(32750))

pp0 <- read.csv(file="PelicanPtResBorder.csv")
