...
[https://www.data.wa.gov.au/](https://www.data.wa.gov.au/){target="_blank"}.

# Downloading shapefile data from `data.wa.gov.au`

*To download some of the files, you may need to register for SLIP (Shared
Location Information Platform).*

### Stepwise procedure

1. **We search in the portal for what we want to find (this example is for an acid sulfate soil risk map)** <br />  ![](./Shapefiles-in-R-maps_files/figure-html/dataWA-search.png)

2. **The search results list will tell us if a shapefile is available (**`SHP`**)** <br /> ![](./Shapefiles-in-R-maps_files/figure-html/choose-dataWA.png)

3. **Open the <u>Data Downloads</u> section and click on `⬇Download`** <br /> ![](./Shapefiles-in-R-maps_files/figure-html/download-dataWA.png)

4. **This takes us to the relevant SLIP page &ndash; click on `⬇Download` again** <br /> ![](./Shapefiles-in-R-maps_files/figure-html/SLIP-download.png)

5. **Hover over the browser downloads and click on the folder icon** <br /> ![](./Shapefiles-in-R-maps_files/figure-html/show-in-folder.png)

6. **Opposite-click on the downloaded zip file and select 'Extract all' (or equivalent)** <br /> ![](./Shapefiles-in-R-maps_files/figure-html/extract-all.png)

7. **Edit the destination folder to your R working directory** <br /> ![](./Shapefiles-in-R-maps_files/figure-html/select-dest-extract.png)

8. **<u>Note</u> that you will extract several files &ndash; you need them all.** <br /> ![](./Shapefiles-in-R-maps_files/figure-html/shapefile-set.png)
...
The `sf` package (Pebesma 2018) function `st_read()` can handle many types of input file, including ESRI shapefiles.

```{r}
library(sf)
assrisk <- 
  st_read("Acid_Sulfate_Risk_Swan_Coastal_Plain_DWER_055.shp")
```
...
```{r shapefile-plot, fig.height=6, fig.width=8, message=FALSE, warning=FALSE, fig.cap="Figure 1: Plot of layer 4 in Swan Coastal Plain Acid Sulfate Soil Risk Map (DWER 2020) converted from shapefile into `sf` object in R."}
