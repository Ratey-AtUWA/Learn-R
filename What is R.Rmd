---
title: "R and RStudio for absolute beginners"
output:
...
>
> --- [Reimann *et al*. (2008) &ndash; *Statistical Data Analysis Explained: Applied Environmental Statistics with R*](https://onesearch.library.uwa.edu.au/permalink/61UWA_INST/1vk1d8f/alma99584168302101){target="_blank"}, p.3.
...
```{r fig-RStudio, fig.height=5.4, fig.width=7.2, fig.align='center', fig.cap="The RStudio window with a very brief explanation of some different sub-panes.", warning=FALSE, error=FALSE, results='hold', echo=FALSE}
fig1 <- readPNG("RStudio.png")
par(mar = c(0,0,0,0))
plot(c(0,1),c(0,1), axes=F, type="n",xaxt="n", yaxt="n",ann=F)
...
2.  **Code-based instructions**. If we want **R** to do something for us, we need to give it instructions. We do this in other software too, commonly by using a mouse or other pointing device to click and select options from a menu. For example, in Excel we can sort a table of data by:
    ...
    In **R**, we can do the same thing, but we would write the instructions in **R** code, such as that shown below:

    You don't have to remember this (yet). The code here is also to illustrate that, in this document, **R** code will be shown in blocks like the one below having a `shaded background and fixed-space font`.
...
```{r fig-1, fig.height=2.41, fig.width=6.16, fig.align='center', fig.cap="Understanding simple R functions and output.", warning=FALSE, error=FALSE, results='hold', echo=FALSE}
fig1 <- readPNG("function_results.png")
par(mar = c(0,0,0,0))
plot(c(0,1),c(0,1), axes=F, type="n",xaxt="n", yaxt="n",ann=F)
addImg(fig1, x = 0.5, y = 0.5, width = 1)
```
...
There are huge numbers of built-in functions in **R** which we can use after first installing **R**, "straight out of the box" (we call this "**base R**"). For instance, see the 
<a href="https://cran.r-project.org/doc/contrib/Baggott-refcard-v2.pdf" target="_blank">R reference card v2 by Matt Baggott</a>.
...
```{r fig-RStudio-file, fig.height=4, fig.width=7.2, fig.align='center', fig.cap="The RStudio window showing the `Source` sub-pane (and the other panes).", warning=FALSE, error=FALSE, results='hold', echo=FALSE}
fig1 <- readPNG("RStudio2.png")
par(mar = c(0,0,0,0))
plot(c(0,1),c(0,1), axes=F, type="n",xaxt="n", yaxt="n",ann=F)
...
<center>![](helpStress.png){width=326 height=300}</center>
