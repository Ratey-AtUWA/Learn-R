...
contain hues most closely resembling the UWA brand colours.
...
the `scico` package are good analogues for the UWA brand colours.
...
The simplest and most obvious option was to try a colour gradient with the two core colours of the UWA brand: <span style="color:white; background-color:#003087;">&nbsp;UWA blue&nbsp;</span> and <span style="background-color:#DAAA00;">&nbsp;UWA gold&nbsp;</span>, plus black and white. A gradient of the desired number of colours was generated using the `colorRampPalette` function in **R**. As can be seen in Figure \@ref(fig:UWA-palette-initial) below, this produced an attractive, full-contrast, gradient which was nearly uniform.

```{r UWA-palette-initial, echo=FALSE, fig.height=9, fig.width=12, message=FALSE, warning=FALSE, out.width="80%", fig.align='center', fig.cap=}
np <- 24
...
plot(1:np, round(greyWt,2), ylim=c(0,1), xlab="Gradient sequence number", 
     ylab="Weighted greyscale value", pch=21, cex=4, bg=UWAgrad, cex.lab=1.4, cex.axis=1.4)
