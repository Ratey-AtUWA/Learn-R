...
library(flextable)      # for making formatted tables
  set_flextable_defaults(theme_fun = "theme_vader", font.size = 10)
...
par(oma=c(0,0,0,0), mar=c(3,3,0.5,0.5), mgp=c(1.5,0.2,0), tcl = -0.2, 
    lend = "square", ljoin = "mitre", lheight=0.85, xpd=T)
plot_tiles(aftiles, mar=c(3,3,0.5,0.5), axes=TRUE) 
...
```{r species-table, message=FALSE, warning=FALSE, paged.print=FALSE, echo=1, results='asis'}
names_table <- 
...
```{r pairwise-Permanova, paged.print=FALSE, echo=2:6}
options(width = 120)
...
s0 <- data.frame(Site=1:16);print(cbind(s0,AF_plainPW_all), row.names=F)
