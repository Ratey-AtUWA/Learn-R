...
The **R** package `vegan` (Oksanen et al. 2022) is designed for analysis of
 descriptive community ecology, including diversity analysis, community ordination and dissimilarity analysis. We will focus on dissimilarity and ordination.
...
eDNAutm <- read.csv(file="eDNA_coords22.csv")
```
...
The following map (Figure \@ref(fig:map-sample-locations)) shows where water
samples for subsequent eDNA analysis were taken within Ashfield Flats Reserve.

```{r map-sample-locations, fig.height=6, fig.width=7.4, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Map of locations where eDNA samples were taken (pink circles) in March 2022 at Ashfield Flats Reserve.", results='hold'}
...
# --==##==--  A D D   D A T A  --==##==--
with(eDNAutm, points(Easting, Northing, col=10, 
                                        bg="orchid1", pch=21, cex=1.5, lwd=2))
with(eDNAutm, text(Easting, Northing, labels=Site, 
                  col="purple", pos=4, cex = 1, font=4, offset = 0.3))
```

<p>&nbsp;</p>

Table \@ref(tab:species-table) below shows all the species identified from their
...
flextable(names_table,cwidth = c(3,2,2)) |> italic(j=1,part="body") |>
  set_caption(caption="List of all the species detected using eDNA, their common names, and abbreviated names used in plots in this document.")
```
...
allSpecies <- eDNA2022[,4:24] # not columns 1:5 which are sample IDs etc.
...
# text(AF_nmds_all$points, labels=eDNA2022$Site, cex=0.9, pos = 4, 
#      col="grey40")
...
AF_pco_scores <- scores(AF_pco_all) # not sure if we need this?!
```
...
palette(c("black",viridis::plasma(9),"white"))
plot(AF_nmds_aquat$points, pch=c(rep(21:25, 2),21)[eDNA2022$Site], 
     xlim = c(-1.2,1.8), 
     bg = seq(1,11)[eDNA2022$Site], 
     cex = 1.4, col.main = "steelblue", main = NA)
# text(AF_nmds_aquat$points, pos = rep(c(1,2,4),6)[eDNA2022$Site], 
#      labels=eDNA2022$Site, cex=1, 
#      col=c(1:17)[eDNA2022$Site])
text(AF_nmds_aquat, display = "species", col = "#20208080", font=3, cex = 0.9)
legend("bottomleft", inset = 0.01, box.col = 3, ncol = 4,
       x.intersp=0.75, title = "Site", 
       legend = seq(1,11), pch=c(rep(21:25, 2),21), 
       pt.bg = seq(1,11), pt.cex = 1.4, cex = 0.9)
```
